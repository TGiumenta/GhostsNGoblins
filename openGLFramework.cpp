#define _CRT_SECURE_NO_DEPRECATE										// Allows the use of strncpy
...
void TerminateApplication (GL_Window* window) noexcept					// Terminate The Application
{
...
void ReshapeGL (int width, int height, int xOffset)						// Reshape The Window When It's Moved Or Resized
{
	...
	glOrtho(-CGame::GetInstance()->GetOrthWidth() + xOffsetDouble, CGame::GetInstance()->GetOrthWidth() + xOffsetDouble,
		-CGame::GetInstance()->GetOrthHeight(), CGame::GetInstance()->GetOrthHeight(), -800.0, 800.0);
...
{
	HWND hwndGlut = FindWindow(NULL, window->init.title);

	long dwStyle = GetWindowLong(hwndGlut, GWL_STYLE);
	dwStyle ^= WS_MAXIMIZEBOX;
	dwStyle ^= WS_SIZEBOX;
	SetWindowLong(hwndGlut, GWL_STYLE, dwStyle);
}

BOOL ChangeScreenResolution (int width, int height, int bitsPerPixel) noexcept	// Change The Screen Resolution
{
...
BOOL CreateWindowGL (GL_Window* window)									// This Code Creates Our OpenGL Window
{
	...
	PIXELFORMATDESCRIPTOR pfd =											// pfd Tells Windows How We Want Things To Be
	{
		...
		static_cast<unsigned char>(window->init.bitsPerPixel),			// Select Our Color Depth
	...
	GLuint PixelFormat = 0;												// Will Hold The Selected Pixel Format
	...
	ReshapeGL (window->init.width, window->init.height, 0);				// Reshape Our GL Window
...
BOOL DestroyWindowGL (GL_Window* window) noexcept						// Destroy The OpenGL Window & Release Resources
{
...
LRESULT CALLBACK WindowProc (HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)														// Evaluate Window Message
	{
		...
		case WM_CREATE:													// Window Creation
		{
			...
			window = static_cast<GL_Window*>(creation->lpCreateParams);
		...
		case WM_SIZE:													// Size Action Has Taken Place
			switch (wParam)												// Evaluate Size Action
			{
				...
				case SIZE_MAXIMIZED:									// Was Window Maximized?
					window->isVisible = TRUE;							// Set isVisible To True
					ReshapeGL (LOWORD (lParam), HIWORD (lParam), 0);	// Reshape Window - LoWord=Width, HiWord=Height
				return 0;												// Return
...
BOOL RegisterWindowClass (Application* application)	noexcept			// Register A Window Class For This Application.
{																		// TRUE If Successful
...
int WINAPI WinMain (_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    *stdin = *hf_in;
	...
	window.init.width			= CGame::GetInstance()->GetScreenWidth();	// Window Width
	window.init.height			= CGame::GetInstance()->GetScreenHeight();	// Window Height
	...
	while (g_isProgramLooping)											// Loop Until WM_QUIT Is Received
	{
		...
		if (CreateWindowGL (&window) == TRUE)							// Was Window Creation Successful?
		{
				...
				while (isMessagePumpActive == TRUE)						// While The Message Pump Is Active
				{
					...
					else												// If There Are No Messages
					{
						...
						else											// If Window Is Visible
						{
							...
							CGame::GetInstance()->DrawScene(tickCount - window.lastTickCount);		// Draw Our Scene
			...
			CGame::GetInstance()->DestroyGame();						// User Defined DeInitialization
